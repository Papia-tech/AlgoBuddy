...
export async function claimSessionPresenter(sessionId, { presenterId, sessionSecret } = {}) {
  const session = await readSession(sessionId);
  if (!session) {
    return { error: "Session not found", status: 404 };
  }

  const expectedSecret = sanitizeSessionText(sessionSecret, 240);
  if (!expectedSecret || expectedSecret !== sanitizeSessionText(session.sessionSecret, 240)) {
    return { error: "Invalid session secret", status: 403 };
  }

  const nextPresenterId = normalizePresenterId(presenterId);
  if (!nextPresenterId) {
    return { error: "Presenter id is required", status: 400 };
  }
...
export async function exchangeRealtimeSubscriptionToken(
  ...
  const channelSecret = crypto
    .createHash("sha256")
    .update(session.sessionSecret)
    .digest("hex")
    .slice(0, 16);

  return {
    realtimeChannel: `collab:${session.id}:${channelSecret}`,
  };
}
